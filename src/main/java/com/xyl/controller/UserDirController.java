...
public class UserDirController {
    ...
    public Result getDirStruct(@PathVariable String id) {
        System.out.println(id);
        UserDir dir = userDirService.getUserDir(id);
        //System.out.println(files);
        return Result.ok().data("dir", dir);
    }

    @ApiOperation(value = "根据传入的路径、名字、和父文件夹的id新建文件夹")
    @PostMapping("setUserDir/{memid}/{name}/{id}")
    public Result setDirStruct(@PathVariable String memid, @PathVariable String name, @PathVariable long id) {
        ...
        //System.out.println(files);
        return Result.error().data("dir", null);
    }

    @ApiOperation(value = "根据传入用户memid、目录路径url、和父文件夹id的删除文件夹")
    @PostMapping("deleteDirStruct/{memid}/{id}")
    public Result deleteDirStruct(@PathVariable String memid, @PathVariable long id, @RequestBody String url) {
        UserDir userDir = userDirService.getUserDir(memid);
        TreeNode treeNode = JSON.parseObject(userDir.getMemDir(), new TypeReference<TreeNode>() {
        });
    ...
    public Result updateDirStruct(@PathVariable String memid, @PathVariable String name, @PathVariable long id, @RequestBody String url){
        UserDir userDir = userDirService.getUserDir(memid);
        TreeNode treeNode = JSON.parseObject(userDir.getMemDir(), new TypeReference<TreeNode>() {
        });
        ...
        if (i>0){
            return Result.ok().data("s",treeNode);
        }else {
            return Result.ok();
        }
    }
